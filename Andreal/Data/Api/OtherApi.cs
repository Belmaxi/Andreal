using System.Text;
using AndrealClient.Data.Json.Bandori;
using AndrealClient.Data.Json.Hitokoto;
using AndrealClient.Data.Json.Ycm;
...
internal static class OtherApi
{
    ...
    internal static async Task<YcmResponse> YcmApi(string carType) =>
        JsonConvert
            .DeserializeObject<
                YcmResponse>(await GetString($"https://ycm.chinosk6.cn/get_car?car_type={carType}&time_limit=900&token=L4ETRgHBPUt8KSkvcO"));

    internal static async Task<YcmResponse> AddCarApi(string carType, string roomid, string description, long userid) =>
        JsonConvert
            .DeserializeObject<
                YcmResponse>(await GetString($"https://ycm.chinosk6.cn/add_car?car_type={carType}&room_id={roomid}&description={description}&creator_id={userid}&data_from=AndreaBot&token=L4ETRgHBPUt8KSkvcO"));
